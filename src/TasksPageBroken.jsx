...
import { taskRecommendationEngine } from './TaskRecommendationEngine.js';
...
export function TasksPage({ projectId }) {
  ...
  const generateAIRecommendations = async () => {
    ...
    try {
      ...
      const projectData = {
        ...
        phase: 'development',
        progress: 65,
        team: teamMembers.filter(m => m.id !== 'unassigned'),
        deadline: '2024-03-15',
      ...
      const userHistory = {
        ...
        commonTaskTypes: ['design', 'development', 'testing'],
      ...
      const contextData = {
        recentActivity: [
          { type: 'task_completed', task: 'API integration testing', date: '2024-02-01' },
          { type: 'file_uploaded', file: 'Database schema.sql', date: '2024-01-31' }
        ],
        bottlenecks: taskRecommendationEngine.identifyBottlenecks(tasks),
        milestones: [
          { name: 'Alpha Release', date: '2024-02-15', status: 'upcoming' },
          { name: 'Beta Testing', date: '2024-03-01', status: 'planned' }
        ]
      };

      const recommendations = await taskRecommendationEngine.generateTaskRecommendations(
        projectData,
        userHistory,
        contextData
      );
      ...
      setProjectContext(await taskRecommendationEngine.analyzeProjectContext(projectData, tasks, contextData.recentActivity));
    } catch (error) {
      console.error('Error generating AI recommendations:', error);
      // Use fallback recommendations if AI fails
      const fallbackRecommendations = taskRecommendationEngine.getFallbackRecommendations({
        phase: 'development',
        progress: 65
      });
  ...
  const acceptRecommendation = (recommendation) => {
    const newTask = {
      ...
      status: 'pending',
      assignedTo: recommendation.suggestedAssignee,
    ...
    setAiRecommendations(aiRecommendations.filter(rec => rec.id !== recommendation.id));
  };

  const dismissRecommendation = (recommendationId) => {
    setAiRecommendations(aiRecommendations.filter(rec => rec.id !== recommendationId));
  };
      ...
      {showRecommendations && (
        <div style={{
          ...
          <div style={{
            ...
            >
              {loadingRecommendations ? 'Generating...' : 'Refresh Suggestions'}
            </button>
          </div>
          ...
          {loadingRecommendations && (
            <div style={{
              ...
              <div style={{
                ...
                <span>Analyzing project context and generating intelligent task suggestions...</span>
              </div>
            </div>
          )}
          ...
          {!loadingRecommendations && aiRecommendations.length === 0 && (
            <div style={{
              ...
              No AI recommendations available. Click "Refresh Suggestions" to generate new recommendations based on your project context.
            </div>
          )}
              ...
              {aiRecommendations.map((recommendation) => (
                ...
                >
                  ...
                  <div style={{
                    ...
                    <span style={{
                      background: recommendation.priority === 'high' ? '#dc2626' : 
